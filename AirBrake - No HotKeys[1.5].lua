...
local newFrame = imgui.OnFrame(
    ...
    function(player)
        ...
        local sizeX, sizeY = 600, 380
        imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY), imgui.Cond.FirstUseEver)
        imgui.Begin(u8'Настройки AirBrake | Версия без HotKeys | Version: ' ..thisScript().version, renderWindow, imgui.WindowFlags.NoResize)
...
function main()
	...
	sampAddChatMessage('{FF8C00}[AirBrake] {FFFFFF}Успешно загружен! (No HotKeys) | Активация: /airb', -1)
...
if AirWork[0] then
	...
	if ScrollWork[0] then
		if scroll == 1 then
			speedCar[0] = speedCar[0] + settings.scroll.ScrollSpeedOnVeh
			printStringNow("speed +"..("%.2f"):format(speedCar[0]), 1000)
...
if AirWork[0] then
	...
	if ScrollWork[0] then
		...
		if scroll == 1 then
			speedFoot[0] = speedFoot[0] + settings.scroll.ScrollSpeedOnFoot
			printStringNow("speed +"..("%.2f"):format(speedFoot[0]), 1000)
		elseif scroll == -1 then
			speedFoot[0] = speedFoot[0] - settings.scroll.ScrollSpeedOnFoot
			if speedFoot[0] < 0 then
