...
url = requests.get("https://raw.githubusercontent.com/cloused2/beta-scripts-lua/main/updater_airbrake-s0beit.json") -- Îòïðàâëÿåì GET çàïðîñ ê íàøåé òàáëèöå
...
local newFrame = imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.Begin(u8'Íàñòðîéêè AirBrake | Version: ' .. version, renderWindow, imgui.WindowFlags.NoResize)
			imgui.Text(u8'Íàñòðîéêè AirBrake')
			imgui.Separator()
				if imgui.Checkbox(u8'Âêëþ÷èòü', AirWork) then
					settings.main.enabled = AirWork[0] 
					inicfg.save(settings, IniFilename)
				end
				imgui.SameLine()
				if imgui.Checkbox(u8'Ïåðåêëþ÷åíèå ñêîðîñòè êîëåñèêîì ìûøêè', ScrollWork) then
					settings.main.scroll = ScrollWork[0]
					inicfg.save(settings, IniFilename)
				end
				if imgui.Checkbox(u8'Ïåðåêëþ÷åíèå ñêîðîñòè êíîïêàìè ìûøêè', MouseWork) then
					settings.main.mouse = MouseWork[0]
					inicfg.save(settings, IniFilename)
				end
			imgui.Separator()
			imgui.Text(u8'Íàñòðîéêè ïåðåêëþ÷åíèÿ ñêîðîñòè')
			if imgui.SliderFloat(u8'Ñìåíà ñêîðîñòè OnFoot', speedClickFoot, 0, 0.0500) then
				settings.main.ClickSpeedOnFoot = speedClickFoot[0]
				inicfg.save(settings, IniFilename)
			end
			if imgui.SliderFloat(u8'Ñìåíà ñêîðîñòè Veh', speedClickVeh, 0, 0.0500) then
				settings.main.ClickSpeedOnVeh = speedClickVeh[0]
				inicfg.save(settings, IniFilename)
			end
			imgui.Separator()
			imgui.Text(u8'Íàñòðîéêè ñêîðîñòè')
			if imgui.SliderFloat(u8'Íà÷àëüíàÿ ñêîðîñòü onFoot', speedFoot, 0, 25) then
				settings.main.SpeedFoot = speedFoot[0]
				inicfg.save(settings, IniFilename)
			end
			if imgui.SliderFloat(u8'Íà÷àëüíàÿ ñêîðîñòü onVeh', speedCar, 0, 25) then
				settings.main.SpeedCar = speedCar[0]
				inicfg.save(settings, IniFilename)
			end
			imgui.TextWrapped(u8'Ïåðåêëþ÷åíèå ñêîðîñòè: ËÊÌ - ïðèáàâèòü  |  ÏÊÌ - óáàâèòü  |  Êîëåñèêî ìûøêè ')
        imgui.End()
    end)
...
function main()
	...
	if tonumber(a["update"]) > version then
        sampShowDialog(1234, "Îáíîâëåíèå "..a["update"], "Îáíàðóæåíî îáíîâëåíèå "..a["update"].."!\nÏåðåéäèòå â òåìó ñî ñêðèïòîì íà blast.hk\n\n"..u8:decode(a["news"]), "Ïîíÿë")
    end
	...
	while true do wait(0)
		...
		if AirWork[0] then
			if isKeyJustPressed(0xA1) and not sampIsChatInputActive() then
